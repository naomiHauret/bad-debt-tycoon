...
import { TestHelpers, type TournamentCombat_CombatResolved } from "generated"
...
describe("TournamentCombat contract CombatResolved event tests", () => {
  ...
  const event = TournamentCombat.CombatResolved.createMockEvent({
  ...
  it("TournamentCombat_CombatResolved is created correctly", async () => {
    ...
    const actualTournamentCombatCombatResolved = mockDbUpdated.entities.TournamentCombat_CombatResolved.get(
    ...
    assert.deepEqual(
      actualTournamentCombatCombatResolved,
      expectedTournamentCombatCombatResolved,
      "Actual TournamentCombatCombatResolved should be the same as the expectedTournamentCombatCombatResolved",
    )
  })
})
