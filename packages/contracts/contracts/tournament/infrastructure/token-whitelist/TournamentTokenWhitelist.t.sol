...
contract TournamentTokenWhitelistTest is Test {
    ...
    function test_GetActiveTokensReturnsOnlyActiveTokens() public {
        whitelist.addToken(address(usdc));
        whitelist.addToken(address(pyusd));
        whitelist.addToken(address(gho));

        whitelist.pauseToken(address(pyusd), "PYUSD paused");

        address[] memory activeTokens = whitelist.getActiveTokens();
        assertEq(activeTokens.length, 2);

        // Verify USDC and GHO are in the list, but not PYUSD
        bool hasUsdc = false;
        bool hasGho = false;
        bool hasPyusd = false;

        for (uint i = 0; i < activeTokens.length; i++) {
            if (activeTokens[i] == address(usdc)) hasUsdc = true;
            if (activeTokens[i] == address(gho)) hasGho = true;
            if (activeTokens[i] == address(pyusd)) hasPyusd = true;
        }

        assertTrue(hasUsdc);
        assertTrue(hasGho);
        assertFalse(hasPyusd);
    }

    function test_GetActiveTokensReturnsEmptyWhenAllPaused() public {
        whitelist.addToken(address(usdc));
        whitelist.addToken(address(pyusd));

        whitelist.pauseToken(address(usdc), "Test 1");
        whitelist.pauseToken(address(pyusd), "Test 2");
    ...
    // Scenario: Multiple pauses during crisis
    function test_Scenario_MultipleTokenCrisis() public {
        whitelist.addToken(address(usdc));
        whitelist.addToken(address(pyusd));
        whitelist.addToken(address(gho));

        // Crisis 1: PYUSD issue
        whitelist.pauseToken(address(pyusd), "PYUSD depeg");

        address[] memory active1 = whitelist.getActiveTokens();
        assertEq(active1.length, 2);

        // Crisis 2: GHO issue
        whitelist.pauseToken(address(gho), "GHO oracle failure");

        address[] memory active2 = whitelist.getActiveTokens();
        assertEq(active2.length, 1);
        assertEq(active2[0], address(usdc));

        // Recovery: Unpause GHO
        whitelist.unpauseToken(address(gho));

        address[] memory active3 = whitelist.getActiveTokens();
        assertEq(active3.length, 2);
    }
}
